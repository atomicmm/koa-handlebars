...
function regHelpers(helpers) {
    ...
    helpers.forEach(helper => {
        engine.registerHelper(helper.name, helper.fn);
    });
    defer.resolve(helpers.length);
...
module.exports.init = (options) => {
    ...
    Q.all([regHelpers(globalOptions.helpers), regPartials(globalOptions.partialPath)]).done(results => {
        debug(`[${results[0]}] helpers,[${results[1]}] partials compiled`);
    });
}
